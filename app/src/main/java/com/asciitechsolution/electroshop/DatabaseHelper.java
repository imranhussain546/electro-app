...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "mangatradios.db";
    ...
    public ArrayList<String> getCartList() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (count.equals("0")) {
                    db.execSQL("DELETE FROM " + TABLE_ORDER_NAME + " WHERE " + PID + " = ?", new String[]{ cursor.getString(cursor.getColumnIndexOrThrow(PID))});
    ...
    public void AddOrderData( String pid, String qty) {
        try {
            if (!CheckOrderExists( pid).equalsIgnoreCase("0")) {
                UpdateOrderData( pid, qty);
            } else {
                SQLiteDatabase db = this.getWritableDatabase();
    ...
    public void UpdateOrderData( String pid, String qty) {
        SQLiteDatabase db = this.getWritableDatabase();
        if (qty.equals("0")) {
            DeleteOrderData( pid);
        } else {
            ContentValues values = new ContentValues();
            values.put(QTY, qty);
            db.update(TABLE_ORDER_NAME, values,   PID + " = ?", new String[]{pid});
        }
        db.close();
    }

    public void DeleteOrderData( String pid) {
        SQLiteDatabase database = this.getWritableDatabase();
        database.execSQL("DELETE FROM " + TABLE_ORDER_NAME + " WHERE " + PID + " = ?", new String[]{pid});
        database.close();
    }

    public String CheckOrderExists(String pid) {

        String count = "0";
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_ORDER_NAME + " WHERE " + PID + " = ?", new String[]{pid});
        if (cursor.moveToFirst()) {
            count = cursor.getString(cursor.getColumnIndex(QTY));
            if (count.equals("0")) {
                db.execSQL("DELETE FROM " + TABLE_ORDER_NAME + " WHERE " + PID + " = ?", new String[]{pid});
